...
(function() {
  ...
  const HTMX = {
    ...
    processElement(element) {
      const elements = element.querySelectorAll('[hx-get], [hx-post], [hx-put], [hx-delete], [hx-patch]');
      elements.forEach(el => this.setupElement(el));
      
      if (element.hasAttribute && (element.hasAttribute('hx-get') || element.hasAttribute('hx-post') || 
          element.hasAttribute('hx-put') || element.hasAttribute('hx-delete') || element.hasAttribute('hx-patch'))) {
    ...
    setupElement(el) {
      ...
      triggers.forEach(trig => {
        ...
        el.addEventListener(eventName, (e) => {
          if (eventName === 'submit' || (eventName === 'click' && el.tagName === 'FORM')) {
            e.preventDefault();
          }
          ...
          this.makeRequest(el, method, url, target, swap, indicator, vals, include, pushUrl);
    ...
    parseModifiers(modifiers) {
      ...
      modifiers.forEach(mod => {
        ...
        if (mod.startsWith('delay:')) options.delay = parseInt(mod.split(':')[1]);
        if (mod.startsWith('throttle:')) options.throttle = parseInt(mod.split(':')[1]);
      });
      return options;
    },
    ...
    async makeRequest(el, method, url, targetSelector, swap, indicatorSelector, vals, include, pushUrl) {
      ...
      try {
        ...
        targetEl.dispatchEvent(new CustomEvent('htmx:afterSwap', { 
          detail: { target: targetEl, swap } 
        }));
        
      } catch (error) {
        console.error('HTMX: Request failed:', error);
        targetEl.dispatchEvent(new CustomEvent('htmx:error', { 
          detail: { error } 
        }));
      } finally {
        if (indicatorEl) {
